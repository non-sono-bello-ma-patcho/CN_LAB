...
int pivoting_parziale(float** matrice,const unsigned int *l_x,const unsigned int *l_y,const unsigned int *c_x,const unsigned int *c_y);
...
/*passo1*/
    ...
    #ifdef STAMPA_OPERAZIONI
    printf("%u)eseguo algoritmo per pivot_parziale sulla sotto_matrice a partire dalla cella %dX%d\n",number_op++,c_x+1,c_y+1);
    #endif
	i=pivoting_parziale(matrice,&l_x,&l_y,&c_x,&c_y);
	
	if(i==l_y)/*non ha trovato un valore maggiore del pivot già impostato*/
	{
	    #ifdef STAMPA_OPERAZIONI
        printf("%u)nessuna riga trovata!\n",number_op++);
        #endif
        if(matrice[c_x][c_y]==0)/*colonna composta interamente da 0*/
        {
	        goto punto3;
	    }
	}
...
/*restituisce indice della riga con cui ho fatto swap*/
int pivoting_parziale(float** matrice,const unsigned int *l_x,const unsigned int *l_y,const unsigned int *c_x,const unsigned int *c_y)
{
    int i=*c_y;
    int i_max=-1;
    float v_max=matrice[*c_x][i];
    
    while(i<*l_y)/*trovo pivot massimo della sotto matrice a partire dagli indici c_x e c_y*/
	{
	    if(matrice[*c_x][i]>v_max)
	    {
	        i_max=i;
	        v_max=matrice[*c_x][i_max];
	    }
	    i++;
	}
if(i_max>=0){return i_max;/*ho trovato un valore maggiore*/}
else{return i;}/*non ho trovato un valore massimo rispetto al pivot iniziale. Il main dovrà inoltre controllare che il pivot non sia 0*/
}
