...
unsigned int l_x,l_y,l_vector,c_x=0,c_y=0,riga_base,i;
float** matrice;
float* vettore;
float parametro;
...
/*allocca la memoria*/
do{matrice=init_matrix(&l_x,&l_y);}while(matrice==NULL);
do{vettore=init_vector(&l_vector);}while(vettore==NULL);
/*inizializza i valori*/
while(fill_vector(vettore,&l_vector));
if(perturba_vettore(vettore,&l_vector)<0){perror("Impossibile perturbare il vettore:");}
#ifdef DEBUG
stampa_vettore(vettore,&l_vector);
#endif
/*inizializza i valori*/
while(fill_matrix(matrice,&l_y,&l_x));
#ifdef DEBUG
